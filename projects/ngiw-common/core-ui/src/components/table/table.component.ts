...
export class TableComponent {
  ...
  ngiwIsLoading = input<boolean>();
  ngiwIsAdding = input<boolean>();
  //@Input() ngiwBindKeyEvents: boolean = true;
  ngiwCellCtxMenu = input({});
  @ContentChild('ngiwHeader', { static: false })
  headerTemplateRef!: TemplateRef<any>;
  @ContentChild('ngiwBody', { static: false })
  bodyTemplateRef!: TemplateRef<any>;
  @ContentChild('ngiwContextMenu', { static: false })
  contextMenuTemplateRef!: TemplateRef<any>;
  ...
  openCustomCtxMenu: { row: number; col: number } = {
    row: null as any,
    col: null as any,
  };
  isScrolling = false;
  ...
  handleContextMenu(event: any, row: any, col: any) {
    if (!this.ngiwCellCtxMenu()) return;
    event.preventDefault();
    this.openCustomCtxMenu = {
      row: row + this.YRangeBegin,
      col: col + this.XRangeBegin,
    };
  ...
  handleKeyDown(e: any, row = 0, col = 0) {
    ...
    if (currTimestamp - this.lastKeyPressTimestamp < 50) return;
    switch (keyCode) {
      case 37:
        this._goArrowLeft();
        break;
  ...
  private goArrowDown() {
    if (this.focusCoordinates.row >= this.ngiwRows() - 1) return;
    this.focusCoordinates.row++;
    this.arrowsKeysObservable.next({
      type: 'down',
      yIndex: this.focusCoordinates.row,
      xIndex: this.focusCoordinates.col,
    });
    this._setActiveCell();
    this.detectChanges();
  }
  ...
  private _goArrowUp() {
    if (this.focusCoordinates.row <= 0) return;
    this.focusCoordinates.row--;
    this.arrowsKeysObservable.next({
      type: 'up',
      yIndex: this.focusCoordinates.row,
  ...
  private _goArrowLeft() {
    if (this.focusCoordinates.col >= this.ngiwCols() - 1) return;
  ...
  private _goArrowRight() {
    if (this.focusCoordinates.col <= 0) return;
    this.focusCoordinates.col--;
    this.arrowsKeysObservable.next({
      type: 'right',
      yIndex: this.focusCoordinates.row,
  ...
  onGetXRange(e: any) {
    ...
    Array.from(
      { length: len - incremente },
      (x, i) => (this.XRange[i] = i + incremente),
    );
    this.XRangeBegin = 0;
  }
  ...
  onMouseUp() {
    if (this.changedWidth) {
      ...
      this.widthService.setWidth(
        this.changedWidth.index,
        this.changedWidth.newWidth,
      );
  ...
  isOverlayOpen = (row: any, col: any) => {
    return (
      this.openCustomCtxMenu.row === row + this.YRangeBegin &&
  ...
  showCloseOverlay = (row: any) => {
    return (
      this.contextMenuTemplateRef &&
      this.ngiwRows() - row + this.YRangeBegin > 0
    );
  };
  ...
  private _setActiveCell() {
    this.activeCell = {
      row: this.focusCoordinates.row,
      col: this.focusCoordinates.col,
    };
  }
}
